...
<script type="text/javascript">
  var url = '<?php echo base_url() ?>index.php/api/post/node_list/<?php echo $campaign->post_id ?>';
  ...
  <div class="col-sm-12">
    <div class="panel panel-primary">
      <div class="panel-heading">
        <h3 class="panel-title"><?php echo $campaign->post_name ?>
          ...
          <table class="table table-striped table-hover table-bordered text-center">
              ...
              <tbody infinite-scroll="loadNextPage()" infinite-scroll-distance="1" infinite-scroll-disabled="loading_records">
                  <tr ng-cloak ng-repeat="record in records | orderBy:sortType:sortReverse | filter:searchtable">
                    ...
                    <td><a href="<?php echo base_url() ?>index.php/node/index/{{ record.node_id }}">{{ record.page_name}}</a></td>
                    <td><label class="label label-{{ record.post_status_label}}" title="{{ record.post_status_title }}">{{ record.post_status_text }}</label></td>
                    <td>
...
jQuery(document).ready(function($){
  ...
  $(document).on('click', '#delete_post_btn', function(){
    if($('input[id="del_fb"]:checked').length > 0)
      window.location.href = $(this).attr('href') + "/true/campaign";
    else
      window.location.href = $(this).attr('href') + "/0/campaign";
    return false; 
  });
});
</script>
